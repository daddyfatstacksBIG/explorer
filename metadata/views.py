...
def add_metadata_to_tx(request, coin_symbol, tx_hash):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if results is True:
                msg = _(
                    '<pre>%(key)s</pre>-><pre>%(value)s</pre> succesfully uploaded to %(upload_string)s (<a href="#metadata">scroll down</a>)'
                    % {
                        "key": metadata_key,
...
def add_metadata(request, coin_symbol):
    ...
    msg = _(
        "%(cs_display)s transaction %(tx_hash)s from latest block (%(latest_block_num)s) randomly selected"
        % {
