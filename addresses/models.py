...
class AddressSubscription(models.Model):
    ...
    unsubscribed_at = models.DateTimeField(
        blank=True, null=True, db_index=True, help_text="User disabled"
    )
    disabled_at = models.DateTimeField(
        blank=True, null=True, db_index=True, help_text="Admin disabled"
    )
    coin_symbol = models.CharField(
        choices=COIN_CHOICES, max_length=16, null=False, blank=False, db_index=True
    )
    b58_address = models.CharField(
        blank=False, null=False, max_length=64, db_index=True
    )
    ...
    auth_user = models.ForeignKey(
        "users.AuthUser", blank=False, null=False, on_delete=models.CASCADE
    )
    blockcypher_id = models.CharField(
        max_length=64, null=False, blank=False, db_index=True
    )
    address_forwarding_obj = models.ForeignKey(
        "addresses.AddressForwarding", blank=True, null=True, on_delete=models.CASCADE
    )
...
class AddressForwarding(models.Model):
    ...
    auth_user = models.ForeignKey(
        "users.AuthUser", blank=True, null=True, on_delete=models.CASCADE
    )
    blockcypher_id = models.CharField(
        max_length=64, null=False, blank=False, db_index=True
    )
